...
def get_timing_point(str, prev_timing_point=None):
    ...
    try:
        ...
        if float(ps[1]) > 0:       # BPM change or SCROLL speed change
            ...
            assert False
        ret["beats"] = int(ps[2]) # mesure change
        ret["GGT"] = (ps[-1] != '0')
    except:
...
def get_note(str):
    ...
    if type == 1: # circle
        ...
        ret.append((END, get_real_offset(int(ps[-1]))))
...
def osu2tja(filename):
    ...
    if not filename.endswith(".osu"):
        print "input file should be *.osu."
        return
    ...
    for line in fp:
        ...
        if osu_ver_str == "":
            osu_ver_str = line
            if not ignore_format_ver and osu_ver_str != "osu file format v9":
                assert False, "Only support osu file format v8/9 at the moment."
                return
...
def convert(filename):
    assert isinstance(filename, basestring)
    assert filename.endswtih(".osu") or filename.endswith(".tja")
...
if __name__ == "__main__":
    global show_head_info, ignore_format_ver
    import sys

    show_head_info = ("debug" in sys.argv)
    ignore_format_ver = ("ignore_version" in sys.argv)
    osu2tja(sys.argv[1])
