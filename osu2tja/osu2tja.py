...
CMD_SCROLLCHANGE = '#SCROLL %f'
CMD_BPMCHANGE = '#BPMCHANGE %f'
CMD_GOGOSTART = '#GOGOSTART'
CMD_GOGOEND = '#GOGOEND'
CMD_MEASURECHANGE = '#MEASURE %d/%d'
CMD_DELAY = '#DELAY %f'
...
def get_donkatsu_by_sound(sound):
	if sound == HITSND_NORMAL:
		...
		HITSND_FINSIH | HITSND_CLAP | HITSND_WHISTLE):
...
def write_incomplete_bar(tm, bar_data, begin, end):
	...
	if len(bar_data) == 0: # 利用DELAY指令忽悠过去
		print make_cmd(CMD_DELAY, (end-begin) / 1000.0)
		return
...
#			print begin, int(begin + beat_cnt * 60000.0 / tm["bpm"]), end
			if beat_cnt >= min_beat_cnt and \
				...
				delay_time = end - int(begin+ beat_cnt * 60000.0 / tm["bpm"])
				...
				if delay_time >= 1:
					print make_cmd(CMD_DELAY, delay_time/1000.0)
				return
...
def osu2tja(filename):
	...
	for line in fp:
		...
		if osu_ver_str == "":
			osu_ver_str = line
			osu_format_ver = int(osu_ver_str[len(OSU_VER_STR_PREFIX):])
	...
	for tm in timingpoints:
		if tm["scroll"] != cur_scroll:
			commands_within.append((tm["offset"], CMD_SCROLLCHANGE, tm["scroll"]))
		if tm["GGT"] != cur_ggt:
			commands_within.append((tm["offset"], 
				tm["GGT"] and CMD_GOGOSTART or CMD_GOGOEND))
...
#			write_a_measure()
			if int(end) == int(bar_offset_begin + bar_max_length):
				...
				if timingpoints[tm_idx]["redline"]:
					...
					print make_cmd(CMD_BPMCHANGE, curr_bpm)				
				else:
				...
				# add new commands
				print make_cmd(CMD_MEASURECHANGE, measure, 4)
